...
class MRIEvaluator:
    ...
    def evaluate_folders(self, generated_dir, ground_truth_dir):
        ...
        gen_files = sorted([f for ext in exts for f in glob.glob(os.path.join(generated_dir, ext))])
        gt_files = sorted([f for ext in exts for f in glob.glob(os.path.join(ground_truth_dir, ext))])

        if len(gen_files) != len(gt_files):
            print(f"Warning: File count mismatch. Gen: {len(gen_files)}, GT: {len(gt_files)}")
        ...
        for gen_path, gt_path in zip(gen_files, gt_files):
            ...
            try:
                ...
                metrics_sum['HFEN'] += self.compute_hfen(img_gen, img_gt)
                metrics_sum['NMSE'] += self.compute_nmse(img_gen, img_gt)
                count += 1
