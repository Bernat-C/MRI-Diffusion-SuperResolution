...
def main():
    ...
    parser.add_argument("--source", type=str, required=True, help="Folder containing .npz files")
    parser.add_argument("--dest", type=str, required=True, help="Output folder for the HF dataset")
    args = parser.parse_args()

    # Create directory structure
    images_dir_hr = os.path.join(args.dest, "hr_images")
    images_dir_lr = os.path.join(args.dest, "lr_images")
    os.makedirs(images_dir_hr, exist_ok=True)
    os.makedirs(images_dir_lr, exist_ok=True)

    metadata_path = os.path.join(args.dest, "metadata.jsonl")
    
    files = glob.glob(os.path.join(args.source, "*.npz"))
    ...
    with open(metadata_path, 'w') as json_file:
        for filepath in tqdm(files):
            try:
                ...
                # Save Images
                hr_img.save(os.path.join(args.dest, hr_rel_path))
                lr_img.save(os.path.join(args.dest, lr_rel_path))
    ...
    print(f"Conversion complete! Dataset saved to: {args.dest}")
